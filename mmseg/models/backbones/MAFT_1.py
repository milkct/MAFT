...
from mmseg.utils import AKConv
from mmseg.utils import MCALayer,RFAConv
from .resnet import  *
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# from Active.models import activemlp
...
class Conv2d_BN(nn.Module):
    ...
    def __init__(self,in_ch, out_ch,kernel_size=1,stride=1,pad=0,dilation=1,groups=1,bn_weight_init=1,norm_layer=nn.BatchNorm2d,
            act_layer=None,
    ):
        super().__init__()
...
class AFFormer(BaseModule):
    def __init__(self,img_size=224,num_stages=4,num_path=[4, 4, 4, 4],num_layers=[1, 1, 1, 1],embed_dims=[64, 128, 256, 512],
        mlp_ratios=[8, 8, 4, 4], num_heads=[8, 8, 8, 8], drop_path_rate=0.0, in_chans=3,num_classes=1000,strides=[4, 2, 2, 2],
        ...
        self.stem = nn.Sequential(Conv2d_BN(in_chans, embed_dims[0] // 2, kernel_size=3, stride=2, pad=1,act_layer=nn.Hardswish,),
            Conv2d_BN(embed_dims[0] // 2, embed_dims[0], kernel_size=3,stride=2,pad=1,act_layer=nn.Hardswish,),)
        ...
        self.conv1 = Conv2d_BN(in_chans, embed_dims[0]//2, kernel_size=3, stride=2, pad=1,act_layer=nn.Hardswish,)
        # self.conv2 = gnconv(embed_dims[0])
        self.AK = AKConv(embed_dims[0]//2,embed_dims[0],4)
        self.RFA = RFAConv(2048,216,1)
        self.MCA = MCALayer(216)


        # self.SST = SST(inp_channels=embed_dims[0],dim=embed_dims[0],window_size=8,depths=[6, 6, 6, 6, 6, 6],num_heads=[6, 6, 6, 6, 6, 6],mlp_ratio=2,qkv_bias=True, qk_scale=None,drop_path_rate=0.1,bias=False)
        # self.conv2 = gnconv(embed_dims[0])
        # self.EVC = EVCBlock(216,216)
